	...
	function cueScrollElements() {	
		...
		for (i=0; i<bkgdLen; i++) {
			makeVisibleInScrollRange(
		    	...
		    	$(document).scrollTop(), 
		    	// from pos
		    	(headerHeight - windowHeight) * i / bkgdLen, 
		    	// to pos
		    	headerHeight,
		    	// note
		    	'.backgrounds .background' + i
			...
			makeVisibleInScrollRange(
		    	...
		    	(headerHeight - windowHeight) * i / bkgdLen, 
		    	// to pos
		    	(headerHeight - windowHeight) * (i + 1) / bkgdLen,
		    	// note
		    	'.footer-backgrounds .footer-background' + i
	    	);
		}
	    ...
	    makeVisibleInScrollRange(
	    	...
	    	$('.backgrounds .stars1'), 
	    	// trigger element position
	    	$(document).scrollTop() + $('.banner-wrap').offset().top,
	    	// from pos
	    	-1, 
	    	// to pos
	    ...
	    makeVisibleInScrollRange(
	    	...
	    	-1, 
	    	// to pos
	    	$('.banner-wrap').height() - (windowHeight) - 200,
	    	// note
	    	'.banner .title'
    	);
	    //
	    ...
	    makeVisibleInScrollRange(
	    	...
	    	100, 
	    	// to pos
	    	$('.banner-wrap').height() - windowHeight,
	    	// note
	    	'.banner .quick-links'
    	);
	    //
